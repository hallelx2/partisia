/*
Adapted from PartisiaBlockchain Documentation - https://partisiablockchain.gitlab.io/documentation/smart-contracts/how-to-create-a-vote-from-a-smart-contract.html

This is an example of a voting smart contract written in rust and intended to be ran on the 
partisia blockchain network.

This system provides a way of performing multiparty computations without revealing individual
data, only the intended results. For example, On a normal public blockchain where votes take 
place, traditional smart contracts reveal the choices of the voters which means you can know
who a voter voted for.

This can be very risky for the voter at times as his choices are not secure all in the name of
verification and trying to be open. But what can be done to solve this probem? That is where 
partisia blockchain comes in to play. It integrates multiparty computations into it's layer one
blockchain, making room for secure computations and easy verifiability of data without revealing
the components of that data. 

This way, computing can be kept safe without compromising the process of verification of the integrity
of data.
*/

// Importing the necessary libraries and types needed for the voting smart contract to run
